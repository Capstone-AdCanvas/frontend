...
function Step4({ setCurrentStep, setHideStepBar, selectedImage }) {
  ...
  const handleFontComboClick = (combo) => {
    ...
    const [font, color, sizeWithExt] = combo.name
      .replace(".png", "")
      .split("_");
    setTextStyle({
      font,
  ...
  const handleNextStep = async () => {
    ...
    try {
      const overlays = addedTexts.map((text) => {
        // 화면 좌표 → 실제 이미지 좌표 변환
        const x = Math.round(
          (text.position.x / renderedSize.width) * imgNaturalSize.width
        );
        const y = Math.round(
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = blobUrl;
      link.download = `${downloadName}.png`; // 확장자 필요 시 조정
      ...
      alert("이미지 이름 저장 및 다운로드 완료");
    } catch (err) {
      console.error("다운로드 실패:", err);
      alert(err.message || "이미지 저장 또는 다운로드에 실패했습니다.");
    }
  };
                ...
                <div className="font-combo__box">
                  ...
                  <div className="font-combo__grid">
                    {fontCombos.map((combo, i) => (
                      <div
                        key={i}
                        className="font-combo__item"
                        onClick={() => handleFontComboClick(combo)}
                        style={{
                          cursor: "pointer",
                          padding: 0,
                          overflow: "hidden",
          ...
          <div className="step4-download-box fade-in">
            {currentImage && (
              <img
                src={currentImage}
                alt="Final"
                className="step4-final-image"
              />
            )}
            <div className="step4-download-info">
              <div className="step4-download-info1">
                <input
              ...
              <div className="step4-download-info2">
                <button
                  className="step4-download-button"
